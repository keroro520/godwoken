...
impl Timepoint {
    const MASK: u64 = 1 << 63;
    const UNSET_LAST_FINALIZED_TIMEPOINT: u64 = u64::MAX;

    // TODO @keroro520 should we remove it?
    /// An unset timepoint, is often used as the placeholder for
    /// GlobalState.last_finalized_block_number, indicates that it has not yet
    /// been assigned a correct value.
    pub fn unset() -> Self {
        Timepoint::from_full_value(Self::UNSET_LAST_FINALIZED_TIMEPOINT)
    }
    ...
    pub fn is_unset(&self) -> bool {
        self.full_value() == Self::UNSET_LAST_FINALIZED_TIMEPOINT
    }
}
